  ...
  """
        ...
        for enemy in self.enemies:
            ...
            if enemy_position is not None and self.evaluate_enemy_in_their_half(enemy_position):
                ...
                if self.in_own_half(game_state.get_agent_position(self.index)):
                    self.enemy_at_gate_way = enemy_position[1]
                else:
                    self.enemy_at_gate_way = None
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                if game_state.has_wall(x, y):
                    grid[x][y] = None
                # food for us?
                elif (x, y) in capsules and len(self.enemy_positions) > 0:
                    try:
                    ...
                    if len(self.enemy_positions) > 0:
                        ...
                        reward = -10 + (self.num_carrying*20)
    ...
    def computeQValueFromValues(self, start_position, result_position, action, capsules):
        ...
        for enemy in self.enemy_positions:
            ...
            if not self.in_own_half(result_position):
                if len(capsules) != 0:
                    if distance_old <= 5 and not self.in_own_half(result_position):
                        if distance_new < distance_old:
                            reward -= ((6 - distance_new) * 1)
                    if (self.action_grid[result_position[0]][result_position[1]][1] * 2 +1) < distance_new:
                        reward -= 10
